...
import (
	...
	"github.com/blu3ph4ntom/codeye/internal/blame"
	"github.com/blu3ph4ntom/codeye/internal/cache"
	"github.com/blu3ph4ntom/codeye/internal/config"
	"github.com/blu3ph4ntom/codeye/internal/git"
	"github.com/blu3ph4ntom/codeye/internal/history"
	"github.com/blu3ph4ntom/codeye/internal/hotspot"
...
func addFlags(cmd *cobra.Command, f *runFlags) {
	...
	cmd.Flags().StringVar(&f.branch, "ref", "", "deprecated alias for --branch")
	...
	cmd.Flags().StringVar(&f.historyInterval, "interval", "week", "deprecated alias for --history-interval")
	...
	cmd.Flags().BoolVar(&f.hotspots, "hotspot", false, "deprecated alias for --hotspots")
	...
	cmd.Flags().BoolVar(&f.versionFlag, "version", false, "print version information")
	_ = cmd.Flags().MarkDeprecated("ref", "use --branch instead")
	_ = cmd.Flags().MarkDeprecated("interval", "use --history-interval instead")
	_ = cmd.Flags().MarkDeprecated("hotspot", "use --hotspots instead")
}

func buildConfig(cmd *cobra.Command, f *runFlags, dir string) (*config.Config, error) {
	cfg, _, err := config.Resolve(f.configPath, dir)
	if err != nil {
		return nil, err
	}

	mergeFlagConfig(cmd, f, cfg)
	return cfg, nil
}

func mergeFlagConfig(cmd *cobra.Command, f *runFlags, cfg *config.Config) {
	...
	override("branch", func() { cfg.Branch = f.branch })
	override("ref", func() { cfg.Branch = f.branch })
	override("author", func() { cfg.Author = normalizeCSVValues(f.author) })
	override("since", func() { cfg.Since = f.since })
	override("until", func() { cfg.Until = f.until })
	override("path-filter", func() { cfg.PathFilter = normalizeCSVValues(f.pathFilter) })
	override("exclude", func() { cfg.Exclude = normalizeCSVValues(f.exclude) })
	override("lang", func() { cfg.Lang = normalizeCSVValues(f.lang) })
	override("no-vendor", func() { cfg.NoVendor = f.noVendor })
	override("no-generated", func() { cfg.NoGenerated = f.noGenerated })
	override("no-tests", func() { cfg.NoTests = f.noTests })
	override("min-lines", func() { cfg.MinLines = f.minLines })
	override("format", func() { cfg.Format = config.NormalizeFormat(f.format) })
	override("sort", func() { cfg.Sort = f.sort })
	override("desc", func() { cfg.Desc = f.desc })
	override("top", func() { cfg.Top = f.top })
	override("no-color", func() { cfg.NoColor = f.noColor })
	override("no-header", func() { cfg.NoHeader = f.noHeader })
	override("wide", func() { cfg.Wide = f.wide })
	override("compact", func() { cfg.Compact = f.compact })
	override("pct", func() { cfg.Pct = f.pct })
	override("progress", func() { cfg.Progress = f.progress })
	override("theme", func() { cfg.Theme = f.theme })
	override("history", func() { cfg.History = f.history })
	override("history-interval", func() { cfg.HistoryInterval = f.historyInterval })
	override("interval", func() { cfg.HistoryInterval = f.historyInterval })
	override("history-limit", func() { cfg.HistoryLimit = f.historyLimit })
	override("blame", func() { cfg.Blame = f.blame })
	override("hotspots", func() { cfg.Hotspots = f.hotspots })
	override("hotspot", func() { cfg.Hotspots = f.hotspots })
	override("contrib", func() { cfg.Contrib = f.contrib })
	override("at", func() { cfg.At = f.at })
	override("speedtest", func() { cfg.Speedtest = f.speedtest })
	override("no-cache", func() { cfg.NoCache = f.noCache })
	override("cache-dir", func() { cfg.CacheDir = f.cacheDir })
	override("workers", func() { cfg.Workers = f.workers })
	override("verbose", func() { cfg.Verbose = f.verbose })
	override("dry-run", func() { cfg.DryRun = f.dryRun })
}
